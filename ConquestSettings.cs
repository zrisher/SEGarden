...
namespace SEGarden.Core {
	...
	public class ConquestSettings {
		...
		private ConquestSettings() {
			if (s_Logger == null)
				s_Logger = new LogicComponents.Logger("Conquest Core", "ConquestSettings");
		...
		private bool loadSettings() {
			...
			try {
				...
				log("Loading settings before MyAPIGateway is initialized. " +
					...
					"loadSettings", LogicComponents.Logger.severity.ERROR);
		...
		public int blockTypeID(BlockType t) {
			...
			log("BlockType not found! Pretending it's the first one.",
				"blockTypeID", LogicComponents.Logger.severity.ERROR);
			return 0;
		}
