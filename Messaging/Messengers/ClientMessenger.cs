...
namespace SEGarden.Messaging {
	...
	public class ClientMessenger {
		...
		public ClientMessenger(bool register = true) {
			log("Started", "ctor");
		...
		public bool requestSettings() {
			...
			try {
				...
				log("Exception occured: " + e, "requestSettings", LogicComponents.Logger.severity.ERROR);
				return false;
			}
		}
		...
		public void incomming(byte[] buffer) {
			...
			try {
				...
				log("Exception occured: " + e, "incomming", LogicComponents.Logger.severity.ERROR);
			}
		}
		...
		private void processDialogResponse(DialogResponse resp) {
			...
			Utility.General.showDialog(resp.Title, resp.Body, "Close");
		}
		...
		private static void log(String message, String method = null, LogicComponents.Logger.severity level = LogicComponents.Logger.severity.DEBUG) {
			if (s_Logger == null)
				s_Logger = new LogicComponents.Logger("Conquest Core", "ResponseProcessor");
